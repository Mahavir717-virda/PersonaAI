...
class GmailConnector(BaseConnector):
    ...
    async def authorize(self, auth_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if code:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if resp.status_code == 200:
                    ...
                    raise RuntimeError(f"Google Token endpoint returned HTTP {resp.status_code}: {resp.text}")

        # Fallback to direct credentials injection if already fetched by the frontend
        ...
        raise ValueError("Missing Google authorization code or access token credentials")
    ...
    async def health(self) -> bool:
        if not self.credentials.get("access_token"):
            return False
        return await self.provider.verify_credentials({"token": self.credentials.get("access_token")})
    ...
    def sync(self, cursor: str | None = None) -> AsyncIterator[dict[str, Any]]:
        async def iterator() -> AsyncIterator[dict[str, Any]]:
            ...
            if cursor:
                try:
                    async for record in self.provider.fetch_messages_since_history(credentials, cursor):
                        yield record
    ...
    async def normalize(self, raw_data: dict[str, Any]) -> Communication:
        # Construct Communication with nested Participants and Attachments
